 ...
 * modified 8/2026 by github.com/g7sim
 */
...
class Controller
{
    ...
    private function render($_template, array $_bag)
    {
        ...
        $file = "{$pth['folder']['plugins']}sitemapper/views/$_template.php";
        
        // SICHERHEITSCHECK: Verhindert den "Kein Wurzel-Element" Fehler
        // Falls die Datei auf dem Linux-Server wegen Groß-/Kleinschreibung nicht gefunden wird
        if (!file_exists($file)) {
            return "<error>Template-Datei nicht gefunden: " . htmlspecialchars($file) . ". Bitte Gross-/Kleinschreibung per FTP pruefen!</error>";
        }
    ...
    private function sitemapIndex()
    {
        ...
        foreach ($this->model->installedLanguages() as $lang) {
            ...
            $sitemap = array(
                // Ohne htaccess nutzen wir das klassische CMSimple Fragezeichen
    ...
    private function sitemaps()
    {
        ...
        $sitemap = array(
            'name' => 'index',
            // Angepasst auf ?sitemap_index.xml
            'href' => CMSIMPLE_ROOT . '?sitemap_index.xml'
        );
        ...
        foreach ($this->model->installedLanguages() as $lang) {
            ...
            $sitemap = array(
                'name' => $lang,
                // Angepasst auf ?sitemap.xml
                'href' => CMSIMPLE_ROOT . $subdir . '?sitemap.xml'
            );
            $sitemaps[] = $sitemap;
        }
        return $sitemaps;
    ...
    private function dispatch()
    {
        ...
        /*
         * OHNE .htaccess prüfen wir den Query-String (alles nach dem Fragezeichen)
         * Das ist die sauberste Lösung für Server ohne URL-Rewriting.
         */
        $qs = isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : '';

        // Sitemap-Index prüfen
        if (strpos($qs, 'sitemap_index.xml') !== false && $sl == $cf['language']['default']) {
            $f = 'sitemapper_index';
            return;
        }

        // Sprach-Sitemap prüfen
        if (strpos($qs, 'sitemap.xml') !== false) {
            $f = 'sitemapper_sitemap';
            return;
        }

        // Alte URLs weiterhin unterstützen (Abwärtskompatibilität)
